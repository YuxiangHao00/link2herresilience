    ...
    def get_state_pollen_count_data(self, format='json'):
        ...
        if self.pollen_grain is not None:
            ...
            if self.duration is not None:
                ...
                df_subset = df_subset[df_subset['week_number'] >= max(curr_week - min(max(int(self.duration), 0), 52), 0)]
                df_subset = df_subset[df_subset['week_number'] <= curr_week]

            if format == 'json':
                return {'pollen_list': self.state_pollen_list,
                        'pollens_count': json.loads(df_subset.to_json(orient="records"))}
            else:
                return df_subset
        else:
            return jsonify({'pollen_list': self.state_pollen_list,

...
def get_model(framework, model_variant, b_verbose=True):
    ...
    if framework in ['keras', 'k']:
        ...
        model = load_model(join("EfficientPose", 'models', 'keras', 'EfficientPose{0}.h5'.format(model_variant.upper())), custom_objects={'BilinearWeights': helpers.keras_BilinearWeights, 'Swish': helpers.Swish(helpers.eswish), 'eswish': helpers.eswish, 'swish1': helpers.swish1})
...
def analyze_image(file_path, model, framework, resolution, lite, b_verbose=True):
    """
...
def analyze(video, file_path, model, framework, resolution, lite, b_verbose=True):
    """
    ...
    # Image analysis
        ...
        coordinates = analyze_image(file_path, model, framework, resolution, lite, b_verbose=b_verbose)
...
def annotate_image(file_path, coordinates):
    ...
    # print("image_draw image: ", image.shape)
...
def perform_tracking(video, file_path, model_name, framework_name, visualize, store,
    ...
    # LOAD MODEL
        ...
        model, resolution = get_model(framework, model_variant, b_verbose=b_verbose)
        if not model:
            return True
        
    # PERFORM INFERENCE
    coordinates = analyze(video, file_path, model, framework, resolution, lite, b_verbose=b_verbose)
    # print(coordinates)

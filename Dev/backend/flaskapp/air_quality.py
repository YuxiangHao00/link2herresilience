...
class AirQuality(Resource):
    ...
    def __init__(self):
        ...
        self.list_aq_parameters = ['pm2_5', 'pm10', 'co', 'no2', 'so2', 'aqc', 'visibility']
        # self.buff = StringIO()
    ...
    def get_state_aq_data(self, state_code=1, format='json'):
        ...
        if self.b_from_files:
            ...
            if self.state is not None:
                ...
                if format == 'json':
                    return json.loads(df_subset[['state', 'date']+self.list_aq_parameters].to_json(orient="records", lines=True))
                else:
                    return df_subset
            else:
...
class SuburbAirQuality(AirQuality):
    ...
    def get_suburb_aq_data(self, format='json'):
        ...
        self.df_curr_state_aq = pd.read_csv(self.datadir_path + self.state +\
                                        '/' + self.state + '_aq_data.csv')\
        
        if self.suburb is not None:
            # select query for suburb data
            df_subset = self.df_curr_state_aq[self.df_curr_state_aq['site'] == self.suburb][['site', 'date']+self.list_aq_parameters]
            ...
            return json.loads(self.df_curr_state_aq[['site', 'date']+self.list_aq_parameters].to_json(orient="records"))

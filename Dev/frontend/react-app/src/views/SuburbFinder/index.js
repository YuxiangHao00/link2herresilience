...
function SuburbFinder() {
    ...
    const fetchPollenData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://52.64.13.221:5002/allergic_pollen/v1/pollens');
    ...
    const handleSearch = async () => {
        if (selectedState && selectedPollen) {
            try {
                const response = await axios.get(`http://52.64.13.221:5002/allergic_pollen/v1/stateCount?state=${selectedState}&pollen=${selectedPollen}`);

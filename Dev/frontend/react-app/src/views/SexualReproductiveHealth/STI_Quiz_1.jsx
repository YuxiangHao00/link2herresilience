...
const HIQuiz1 = (props) => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch('https://link2herresilience.com.au/health_education/v1/quiz?topic=prevention');
      if (!response.ok) {
  ...
  const submitQuiz = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`https://link2herresilience.com.au/health_education/v1/quiz?topic=prevention&questions=[${questionIds.join(',')}]&answers=[${answers.join(',')}]`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
    ...
    <div className="main-page img" style={{ backgroundImage: `url(${bg})` }}>
      ...
      {!quizCompleted ? (
        ...
        <div className="quiz-results">
          ...
          <button onClick={() => props.actType(9)} className="main-btn">
            Return to Module
          </button>
        </div>
      )}
